...
//From far to close for bounary check
let check = function(cells, color, i, j) {
    let boardWidth = cells.size;
    let boardHeight = cells.get(0).size;
    if (arguments.length < 4) throw Error('Missing parameters for cell');
...
let checkAllDirections = (cells, color, i, j) => {
    return directions.map(([di, dj]) => checkFour(cells, color, i, j, stepFactory(di, dj))).some(t => t);
}
